...
from sensor_vision import sensor_vision, sensor_reading, target_localization_both
...
meas_type = ["radial", "bearing", "radial"] #"bearing" or "radial" options
# ------------------------------------------

sensor_locs = [57, 58, 36, 46]#, 53, 40]
target = [40, 60]
...
for i in range(len(sensor_locs)//2):
    ...
    nodes, _ = sensor_vision(sensor, sensor_rad[i], target, meas_type[i])

    # Generate a list of unit vectors
    # These are passed into the localization calculation later
    measurement, seen = sensor_reading(sensor, sensor_rad[i], target, meas_type[i])
    if seen:
        measurements.append(measurement)
        print("radii", measurements) 
...
ax.set_title("Sensor vision of a target - radial")

# Now try localizing a point
target_loc = target_localization_both(measurements, sensor_locs, meas_type)
