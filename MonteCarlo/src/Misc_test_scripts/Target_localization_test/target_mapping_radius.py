...
from sensor_vision import sensor_vision, sensor_reading, target_localization_radius
...
for i in range(len(sensor_locs)//2):
    ...
    measurement, seen = sensor_reading(sensor, sensor_rad[i], target, meas_type)
    if seen:
...
target_loc = target_localization_radius(measurements, sensor_locs)
